...
- 路由必须先声明读取路径：`--route-path direct-ocr` 表示普通 OCR 路径，`--route-path complex-parse` 表示复杂文档解析路径；凡 PaddleOCR 支持的输入，两类路径均优先 PaddleOCR。
- Office、远程网页、当前 PaddleOCR 协议不能提交的远程 URL 才选择 MinerU。
...
- 异步模式支持 `PP-OCRv5` 和 `PaddleOCR-VL-1.6`。Markdown 输出优先使用 `PaddleOCR-VL-1.6`，因为它返回版面块和 Markdown 文本。

### MinerU

- 支持本地 PDF、图片、Office 文档、远程文档 URL。
- 配置 Token 后支持网页 URL。
- 无 MinerU Token 时不调用 MinerU，支持的材料直接使用 PaddleOCR。
