...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--route-path",
        choices=["auto", "direct-ocr", "complex-parse"],
        default=None,
        help="读取路径；direct-ocr 为普通 OCR，complex-parse 为复杂文档解析；PaddleOCR 支持的输入均优先 PaddleOCR",
    )
    ...
    parser.add_argument(
        "--paddle-model",
        choices=["PP-OCRv5", "PaddleOCR-VL-1.6"],
        help="PaddleOCR 异步任务模型；Markdown 输出建议 PaddleOCR-VL-1.6",
    )
    parser.add_argument(
        "--paddle-api-protocol",
...
def build_archive(
    *,
    ...
    archive_base = Path(
        os.environ.get(
            "LEGAL_OCR_ARCHIVE_DIR",
            str(get_skill_root() / "archive"),
        )
    ).expanduser()
    archive_root = archive_base / f"{timestamp}_{sanitize_name(archive_name)}"
...
def main(argv: list[str] | None = None) -> int:
    ...
    route_path = args.route_path or env.get("LEGAL_OCR_ROUTE_PATH", "auto")
    route = choose_backend(source, env, configured_backend, route_path)
